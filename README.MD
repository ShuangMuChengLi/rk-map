...
22. （已修改）框选实现右键取消框选画笔状态。遇到了一个问题  用户开始准备框选  鼠标变成了画笔  突然不想画了   右键取消画笔
...
34. (已修改)点击轨迹上的点，能够高亮，并触发事件，并触发手动播放模式，并将当前播放的index设置为这个点
...
41. (已修改)多个点位  地图居中 -- rkMap.fitViewZoom(pointList); var pointList = [{ id: '1', lonLat: [118.15, 24.51], text: '1'}, {id: '2', lonLat: [118.14, 24.5],text: '2'}
42. (已修改)vecLayer1.getSource().addRkFeatures([{
        id: '11',
...
43. (已修改)删除startManualTrack 、endManualTrack  ；startAutoTrack自动停止手动播放；prevTrack、nextTrack自动停止自动播放；doAction等其他播放设置放在setTrack中；
44. (已修改)缺少轨迹自动播放结束的回调函数
45. (已修改)mapServiceObj的projection和RKMap的projection区别 ——mapServiceObj的projection是view的
...
显示到其他较远的地方。
46. (已修改)不设置projection，updateView失效
47. (已修改)获取某点根据X,Y像素的偏移后的经纬度 - rkMap.setCenter(coor, offset) ,offset = [ - width/2 ,- heigth /2]
48. 点击后出现两个重叠图标
3. vector.addLayerEventListener('moveout'）没触发
5. vecLayer.setFeatureSelected(feature); 和vecLayer.cancelFeatureSelected(feature)添加demo。支持两种参数feature以及ID
--使用方法,传入feature的id 或 feature本身。 vecLayer.setFeatureSelected(id)
6. 发现之前函数名称命名错误：原来函数为vecLayer.getLayerSourse() ,现在改为vecLayer.getLayerSource().原本的getLayerSourse()当前版本也还可以使用。
以后可能会删除这个错误命名的函数。
4. 手动\自动画一条线，高亮线条周边X米范围的点位  -- 使用方法：参考drawTool.html 
...
1. 图标选中后，点击地图其他地方，不要还原样式。图层提供一个方法，手动调用，然后还原。
2. 点击矢量图层，高亮，点击地图空白地方，高亮图标会一闪一闪
3. 点击的高亮，要能够通过vector.cancelFeatureSelected(currentActivePoint);取消高亮
4. 矢量图层点击高亮后 ，清空图层，其他图标消失，但高亮图标还在。点击地图其他地方高亮消失，然后报错
````
RKSelectInteraction.js:113 Uncaught TypeError: Cannot read property 'get' of null
    at getFunc (RKSelectInteraction.js:113)
    at RKSelectInteraction.getSelectMethod (RKSelectInteraction.js:123)
    at RKSelectInteraction.getSelectedFeatureStyle (RKSelectInteraction.js:136)
    at RKSelectInteraction.initSelectedLayerStyle (RKSelectInteraction.js:170)
    at CanvasVectorLayerRenderer.<anonymous> (VectorLayer.js:310)
    at CanvasVectorLayerRenderer.prepareFrame (VectorLayer.js:330)
    at VectorLayer.Layer.render (Layer.js:199)
    at CompositeMapRenderer.renderFrame (Composite.js:100)
    at Map.PluggableMap.renderFrame_ (PluggableMap.js:1264)
    at Map.<anonymous> (PluggableMap.js:176)
````

5. 聚合图层存在类似2、3问题，而且点击地图空白地方，图标会消失，期望不消失
8. （已修改）弹窗隐藏的方法是哪个
9. （已修改）clusterLayer.html  initClusterSelectedFn第一个参数不是一个数组，但demo却去判断length
![Image text](https://github.com/ShuangMuChengLi/rk-map/blob/master/clickFeature.png)
10. （已修改）移除以下代码，使用addEventListener、removeEventListener代替
````
...
11. （已修改）事件名：select movein moveout rightdown

    样式：selectedStyle moveinStyle moveoutStyle rightdownStyle
12. （已修改）ol.ropeok.RKFeatures、new ol.ropeok.RKVectorLayer建议就一个参数
````
...
14. （已修改）demo要简洁些，明了的告诉读者，api怎么调用。不要过度封装
15. （已修改）移除ol.ropeok.RKFeatureStyle。前端不用调用这个方法，只传递一个对象，api内部自行调用。
16. （已修改）jsdemo/demo/drawFeatureByData.html打开默认报错，添加按钮不知道什么功能   点了没效果
17. （已修改）RKDrawFeatureByData  设置了drawLayer为什么还要设置vecSources  建议只传drawLayer
18. （已修改）vecSources建议命名为 vectorSource，drawLayer建议命名为layer。所有key名，统一，便于前端记忆
1. （已修改）图标不设置样式，默认不显示或者显示小圆点。不要依赖图片marker.png
2. （已修改）feature和vector不应该耦合在一起
3. （已修改）图标点击后，出现两个叠加的图标
4. （已修改）缺少弹窗的demo。showPopupById和showPopupModelById合并。showPopupById不行   可以删了。因为调用第二次  会找不到那个ID
5. （已修改）绘制圆形、矩形、任意图形，框选范围内的点位，并高亮
6. （已修改）绘制轨迹，车辆在轨迹上移动，画方圆五百米的圆，框选并高亮范围内点位。
## 优化
